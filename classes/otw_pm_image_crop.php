 ...
 */
class OTWPMImageCropLite {

  /**
   * Image extensions accepted for resize
   */
  private $imgTypes = array("gif", "jpeg", "jpg", "png"); // used to determine image type
   ...
   */
  public function OTWPMImageCropLite() {
	...
	if( otw_init_filesystem() ){
		...
		if( !$wp_filesystem->is_writable( $this->imgBaseDir ) ) {
   ...
   */
  public function resize( $imgData, $resizeWidth, $resizeHeight, $crop = false, $white_spaces = false, $background = false, $format = '' ){
	
	global $otw_pml_image_object, $otw_pml_image_profile;
	
	return set_url_scheme( $otw_pml_image_object->resize( $otw_pml_image_profile, $imgData, $resizeWidth, $resizeHeight, $crop , false, $white_spaces, $background, $format ) );
  } 
  ...
  public function embed_resize( $html, $resizeWidth, $resizeHeight, $crop = false ){
	...
	return $otw_pml_image_object->embed_resize( $otw_pml_image_profile, $html, $resizeWidth, $resizeHeight, $crop );
  }
	
   
  public function resize_old ( $imgData, $resizeWidth, $resizeHeight, $crop = false) {
	...
	if( otw_init_filesystem() ){
		...
		if( $wp_filesystem->exists( $fileTarget . '/' . $newFilename ) ) {
   ...
   */
	public function cacheDir ( $directory ) {
		...
		if( otw_init_filesystem() ){
			...
			if ( !$wp_filesystem->is_dir($baseDirectory) ) {
